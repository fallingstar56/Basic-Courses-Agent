...
def collect_images_and_update_md(part_dir, merged_images_dir, part_index, merged_md_file):
    ...
    支持标准 Markdown 图片语法和 HTML <img> 标签，支持无扩展名模糊匹配。
    """
    # 1. 扫描所有图片文件，建立原始相对路径 -> 新文件名 的映射
    image_exts = ('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp')
    image_map = {}  # 键：相对于 part_dir 的路径（正斜杠），值：新文件名

    for root, _, files in os.walk(part_dir):
        for f in files:
            if f.lower().endswith(image_exts):
                # 获取相对于 part_dir 的路径，统一使用正斜杠
                orig_path = os.path.relpath(os.path.join(root, f), part_dir).replace('\\', '/')
                # 生成唯一新文件名：part_{index}_{原路径中的特殊字符替换为下划线}
    ...
    # 调试输出（可选，正式运行可注释）
    print(f"part_{part_index}: 找到 {len(image_map)} 张图片，映射示例: {list(image_map.items())[:3]}")

    # 2. 处理所有 .md 文件（按页码排序）
    md_files = [f for f in os.listdir(part_dir) if f.endswith('.md')]
    md_files.sort(key=lambda x: int(x.split('_')[1].split('.')[0]) if '_' in x and x.split('_')[1].split('.')[0].isdigit() else 0)
    ...
    for md_file in md_files:
        ...
        # ----- 增强替换逻辑 -----
        # 1) 替换标准 Markdown 图片语法: ![alt](path)
        def replace_md_link(match):
            alt = match.group(1)
            orig_path = match.group(2).strip()
            # 去除可能的前缀 './' 或 '.\'
            orig_path = re.sub(r'^\./', '', orig_path)
            orig_path = orig_path.replace('\\', '/')
            if orig_path in image_map:
                new_path = f"{MERGE_IMAGES_DIR}/{image_map[orig_path]}"
                return f'![{alt}]({new_path})'
            else:
                # 尝试去除文件名后缀进行匹配（应对无扩展名引用）
                base_no_ext = os.path.splitext(orig_path)[0]
                for k in image_map:
                    if os.path.splitext(k)[0] == base_no_ext:
                        new_path = f"{MERGE_IMAGES_DIR}/{image_map[k]}"
                        print(f"part_{part_index}: 模糊匹配成功: '{orig_path}' -> '{k}'")
                        return f'![{alt}]({new_path})'
                # 若仍失败，保留原链接并输出警告
                print(f"警告: part_{part_index} 中未找到图片映射: {orig_path}")
                return match.group(0)

        # 2) 替换 HTML <img> 标签的 src 属性
        def replace_html_img(match):
            tag = match.group(0)
            src_match = re.search(r'src=["\'](.*?)["\']', tag)
            if not src_match:
                return tag
            orig_path = src_match.group(1).strip()
            orig_path = re.sub(r'^\./', '', orig_path)
            orig_path = orig_path.replace('\\', '/')
            if orig_path in image_map:
                new_path = f"{MERGE_IMAGES_DIR}/{image_map[orig_path]}"
                new_tag = tag.replace(src_match.group(0), f'src="{new_path}"')
                return new_tag
            else:
                base_no_ext = os.path.splitext(orig_path)[0]
                for k in image_map:
                    if os.path.splitext(k)[0] == base_no_ext:
                        new_path = f"{MERGE_IMAGES_DIR}/{image_map[k]}"
                        new_tag = tag.replace(src_match.group(0), f'src="{new_path}"')
                        print(f"part_{part_index}: HTML模糊匹配成功: '{orig_path}' -> '{k}'")
                        return new_tag
                print(f"警告: part_{part_index} HTML图片未找到映射: {orig_path}")
                return tag

        # 先替换 Markdown 图片语法
        md_pattern = re.compile(r'!\[(.*?)\]\((.*?)\)')
        content = md_pattern.sub(replace_md_link, content)

        # 再替换 HTML img 标签（非贪婪匹配）
        html_pattern = re.compile(r'<img[^>]*src=["\'][^"\']*["\'][^>]*>', re.IGNORECASE)
        content = html_pattern.sub(replace_html_img, content)

        # 写入合并文件
        merged_md_file.write(content)
...
def main():
    ...
    # 注意：每个 part 的子目录未自动删除，如需清理可添加循环
